...
static inline unsigned long getbit(immense source, int bitno, int nbits);
static inline unsigned long ks(/*immense key, */int n, great * kn);
static inline unsigned long cyfun(unsigned long ir, great k, unsigned long * iout);
...
#endif
   int ii,i,j,k;
   unsigned long ic,shifter,getbit(),ks(), cyfun();
   immense itmp;
   great pg;
    	  ...
    	  //ks(/* key,*/ i, &pg);
    	  {
    		  int m,j,k,l;
    		     ...
    		     for (j=16,k=32,l=48; j>=1; j--,k--,l--) {
    		        pg.r=(pg.r <<= 1) | (unsigned short)
    		            getbit(icd,ipc2[j],28);
    		        pg.c=(pg.c <<= 1) | (unsigned short)
    		            getbit(icd,ipc2[k],28);
    		        pg.l=(pg.l <<= 1) | (unsigned short)
...
int main(void)
{
   ...
   int newkey, isw;
   int know_value;
   klee_make_symbolic(&know_value, sizeof(know_value),
    			"know_value");

   klee_make_symbolic(&ipc1, sizeof(ipc1),
    			"ipc1");
   for (int i = 0; i < 57; i++)
     klee_prefer_cex(ipc1, 0 <= ipc1[i] && ipc1[i] <= 60); // assume ASCII

   /*klee_make_symbolic(&ipc2, sizeof(ipc2),
    			"ipc2");
   for (int i = 0; i < 49; i++)
     klee_prefer_cex(ipc2, 0 <= ipc2[i] && ipc2[i] <= 60); // assume ASCII

   /*klee_make_symbolic(&ip, sizeof(ip),
    			"ip");
   for (int i = 0; i < 65; i++)
     klee_prefer_cex(ip, 0 <= ip[i] && ip[i] <= 60); // assume ASCII

   klee_make_symbolic(&ipm, sizeof(ipm),
    			"ipm");
   for (int i = 0; i < 65; i++)
     klee_prefer_cex(ipm, 0 <= ipm[i] && ipm[i] <= 60); // assume ASCII
*/
   ...
   klee_make_symbolic(&value, sizeof(value),
 			"value");
   newkey = value;
   isw = value;
