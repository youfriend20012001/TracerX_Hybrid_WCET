...
public:
	...
	public:
		...
		bool operator<(const RawAbstractState &b) const {
			//if(!this->state->pc || !b.state->pc) return true;
			return this->state->pc->dest < b.state->pc->dest;
		}
	};
	...
	public:
		...
		void concat(const HSETSummary &b) {
			...
			for (int i = 0; i < maxNode - initial; i++) {
				this->Path[i + initial] = b.Path[i];
			}

			this->Concrete = this->Concrete && b.Concrete;
		}

		bool operator!=(const HSETSummary &b) const {
			bool isSamePath = true;
			for (int i = 0; i < maxNode; i++) {
				if (this->Path[i] != b.Path[i]) {
					isSamePath = false;
					break;
				}
			}
			return (this->WCET != b.WCET) || !isSamePath
		...
		bool equal(const HSETSummary &b) const {
			bool isSamePath = true;
			for (int i = 0; i < maxNode; i++) {
				if (this->Path[i] != b.Path[i]) {
					isSamePath = false;
		...
		void operator=(const HSETSummary &b) {
			for (int i = 0; i < maxNode; i++) {
				this->Path[i] = b.Path[i];
			}
			this->WCET = b.WCET;
			this->Concrete = b.Concrete;
			this->LWCET = b.LWCET;
		...
		HSETSummary clone(int initialPoint) {
			...
			for (int i = initialPoint; i < maxNode; i++) {
				if (i - initialPoint >= 0)
					result.Path[i - initialPoint] = this->Path[i];
			}
			result.Concrete = this->Concrete;
			return result;
		}

		HSETSummary clone(int initialPoint, int endPoint) {
			HSETSummary result;
			result.WCET = this->WCET;
			result.LWCET = this->LWCET;
	...
	public:
		...
		int maxNode;
	public:
		ExecutionPath(HSETSummary summary) {
			depth = 0;
			maxNode = summary.maxNode;
			this->Path = new int[maxNode];
			for(int i = 0; i < maxNode; i++){
				this->Path[i] = summary.Path[i];
				if(summary.Path[i] >= 0) depth = i;
			}
		}
		ExecutionPath(HSETSummary summary, int depth) {
			this->depth = depth;
			maxNode = summary.maxNode;
			this->Path = new int[maxNode];
			for(int i = 0; i < maxNode; i++){
				if(i <= depth) this->Path[i] = summary.Path[i];
				else this->Path[i] = -1;
			}
		}
		bool operator<(const ExecutionPath &b) const {
			for(int i = 0; i < maxNode; i++){
				if(this->Path[i] < b.Path[i]) return true;
				else if(this->Path[i] > b.Path[i]) return false;
			}
			return false;
		}
		bool operator==(const ExecutionPath &b) const {
			if(this->maxNode != b.maxNode) return false;
			if(this->depth != b.depth) return false;
			for(int i = 0; i < maxNode; i++) if(this->Path[i] != b.Path[i]) return false;
			return true;
		}
	...
	public:
		...
		std::map<ExecutionPath, HSETSummary> exactPathDictionary;
		std::map<ExecutionPath, HSETSummary> exactDictionary;
...
public:
	...
	HSETSummary runWithSymbolicExecution(ExecutionState &state,
			HSETSummary guilde,int depth, Executor::HSETAbstractMethods abstractMethod, bool IsAbstractWalk);

	HSETSummary runWithExecutionTree(PTreeNode *runningNode,
				HSETSummary guilde,int depth, Executor::HSETAbstractMethods abstractMethod);
