	...
	if (interpreterOpts.ExeConfig == Interpreter::SymbolicExecution) {
		...
		while (countRun < maxLoop && (endRate > 0.05 || endRate < 0.05) && !HSETInfo.currentWCET.isConcrete()) {
			...
			llvm::errs() << "Run with guide. Chosen path will be (WCET = "
					<< HSETInfo.currentWCET.WCET << "): ";
			for (int i = 0; i < HSETInfo.currentWCET.maxNode; i++) {
				if (HSETInfo.currentWCET.Path[i] >= 0)
					llvm::errs() << HSETInfo.currentWCET.Path[i];
			}
			llvm::errs() << "\n";
			HSETInfo.previousWCET = HSETInfo.currentWCET.clone(0);

			HSETInfo.currentWCET = runWithExecutionTree(processTree->root,
					HSETInfo.currentWCET, 0, AbstractMethods);
			...
			time(&etimer);
			//llvm::outs()
			//<< "End run " << countRun << " , current execution time:"
			//		<< std::clock() - c_start << "\n";
			llvm::errs() << "End run " << countRun << " , number abstract node:"
					<< HSETInfo.rawAbstractDictionary.size() << "\n";
			//llvm::outs()
			//<< "End run " << countRun << " , number path node:"
			//<< HSETInfo.exactPathDictionary.size() << "\n";

			llvm::errs() << "End run " << countRun << " , number exact node:"
								<< HSETInfo.exactDictionary.size() << "\n";

			if (interpreterOpts.ExeConfig == Interpreter::HybridAbstractExecution) {
				if(HSETInfo.previousWCET.equal(HSETInfo.currentWCET))
					break;
			} else {
				if(HSETInfo.currentWCET.isConcrete()) break;
			}
		}

		if (HSETInfo.currentWCET.isConcrete()) {
			llvm::errs() << "Find concrete path!" << "\n";
			if (HSETInfo.currentWCET.Path[0] >= 0) {
...
Executor::HSETSummary Executor::runWithAbstract(ExecutionState &initialState,
	...
	while (state.pc) {
		...
		if (rawMemoriedState != HSETInfo.rawAbstractDictionary.end()) {
			...
			for (int i = 0; i < resultHSET.maxNode; i++) {
				resultHSET.Path[i] = (rawMemoriedState->second).Path[i];
			}
			/*
			 ...
			 for (int i = 0; i < 100; i++) {
			 if (resultHSET.Path[i] >= 0)
			 llvm::errs() << resultHSET.Path[i];
			 }
			 llvm::errs() << "\n";
			 llvm::errs() << (*(state.pc)).dest;
			 llvm::errs() << "\n";*/
...
Executor::HSETSummary Executor::runWithExecutionTree(PTreeNode *runningNode,
		Executor::HSETSummary guilde, int depth,
	...
	std::map<ExecutionPath, HSETSummary>::iterator tmpExactCachedState;
	HSETSummary abstractPath = guilde.clone(0, depth);
	if (runningNode->left || runningNode->right) {
		llvm::errs() << "Meet branch instruction, current WCET is "
				<< resultHSET.WCET << "\n";
		HSETSummary trueHSET, falseHSET;

		if (guilde.Path[depth] == 1 || guilde.Path[depth] < 0) {
			if (runningNode->right) {
				...
				llvm::errs() << "Infeasible true branch.3 \n";
				trueHSET.WCET = -99999;
			}

			if (runningNode->left) {
				HSETSummary correspondingPath = guilde.clone(0);
				correspondingPath.Path[depth] = 0;
				tmpExactCachedState = HSETInfo.exactPathDictionary.find(
						Executor::ExecutionPath(correspondingPath, depth));
				if (tmpExactCachedState != HSETInfo.exactPathDictionary.end()) {
					llvm::errs() << "Find explored path, reuse :"
							<< (tmpExactCachedState->second).WCET << "\n";
					llvm::errs() << "Path:";
					for (int i = 0; i <= depth; i++)
						llvm::errs() << correspondingPath.Path[i];
					llvm::errs() << "\n";

					falseHSET = (tmpExactCachedState->second).clone(0);
				} else {
					if (runningNode->left->data) {
						if(interpreterOpts.ExeConfig == Interpreter::HybridAbstractExecution){
							llvm::errs()<<"Temporary Symbolic Tree";
							abstractPath.Path[depth] = 0;

							falseHSET = runWithSymbolicExecution(*(runningNode->left->data), abstractPath, depth + 1,
								abstractMethod, true);
						...
						llvm::errs() << "Infeasible false branch.1 \n";
			...
			if (runningNode->right) {
				HSETSummary correspondingPath = guilde.clone(0);
				correspondingPath.Path[depth] = 1;
				tmpExactCachedState = HSETInfo.exactPathDictionary.find(
						Executor::ExecutionPath(correspondingPath, depth));
				if (tmpExactCachedState != HSETInfo.exactPathDictionary.end()) {
					llvm::errs() << "Find explored path, reuse :"
					...
					trueHSET = (tmpExactCachedState->second).clone(0);
				} else {
					if (runningNode->right->data) {
						if(interpreterOpts.ExeConfig == Interpreter::HybridAbstractExecution){
							abstractPath.Path[depth] = 1;
							llvm::errs()<<"Temporary Symbolic Tree";
							trueHSET = runWithSymbolicExecution(*(runningNode->right->data), abstractPath, depth + 1,
								abstractMethod, true);
						} else if(interpreterOpts.ExeConfig == Interpreter::HybridSymbolicExecution)
							trueHSET = runWithAbstract(*(runningNode->right->data),abstractMethod,false);
					} else {
						llvm::errs() << "Infeasible true branch.1 \n";
						trueHSET.WCET = -99999;
					}
				}
				llvm::errs() << resultHSET.WCET << " --- True WCET is "
	...
	if ((depth - 1) >= 0) {
		std::pair<ExecutionPath, HSETSummary> curExactInfo(
				Executor::ExecutionPath(guilde, depth - 1), resultHSET);
		std::map<ExecutionPath, HSETSummary>::iterator exactCachedState;
		exactCachedState = HSETInfo.exactPathDictionary.find(
				Executor::ExecutionPath(guilde, depth - 1));
		if (exactCachedState != HSETInfo.exactPathDictionary.end()) {
			HSETInfo.exactPathDictionary.erase(exactCachedState);
		}
		llvm::errs() << "Saving Path:";
		for (int i = 0; i < depth; i++)
			llvm::errs() << guilde.Path[i];
...
Executor::HSETSummary Executor::runWithSymbolicExecution(ExecutionState &state,
	...
	std::map<ExecutionPath, HSETSummary>::iterator tmpExactCachedState;
	 ...
	 /*std::map<ExecutionPath, HSETSummary>::iterator exactCachedState;
	 exactCachedState = HSETInfo.exactDictionary.find(Executor::ExecutionPath(guilde, depth));
	 if (exactCachedState != HSETInfo.exactDictionary.end()) {
		 llvm::errs()<< "Find exact explored path, reuse :" << (exactCachedState->second).WCET<<"\n";
		 resultHSET = (exactCachedState->second).clone(0);
		 return resultHSET;
	 }*/
...
//state.executionTime = 0;
	while (state.pc) {
				...
				if(IsAbstractWalk){
					...
					if(branches.first && branches.second){
						...
						tempHSET = runWithSymbolicExecution(*branches.first,
								abstractPath, depth + 1, abstractMethod, true);
						resultHSET.updateNextNode(1);
					...
					if (branches.first) {
						...
						if (guilde.Path[depth] == 1 || guilde.Path[depth] < 0) {
							...
							correspondingPath.Path[depth] = 1;
							tmpExactCachedState = HSETInfo.exactPathDictionary.find(
									Executor::ExecutionPath(correspondingPath,
											depth));
							if (tmpExactCachedState
									!= HSETInfo.exactPathDictionary.end()) {
								llvm::errs() << "Find explored path, reuse :"
										<< (tmpExactCachedState->second).WCET
										<< "\n";
								llvm::errs() << "Path:";
								...
								abstractPath.Path[depth] = 1;
								trueHSET = runWithSymbolicExecution(*branches.first, abstractPath, depth + 1,
										abstractMethod, true);
							}
						}
						llvm::errs() << resultHSET.WCET << " --- True WCET is "
					...
					if (branches.second) {
						...
						if (guilde.Path[depth] == 1) {
							...
							if (tmpExactCachedState
								...
								abstractPath.Path[depth] = 0;
								falseHSET = runWithSymbolicExecution(*branches.second, abstractPath, depth + 1,
										abstractMethod, true);
							}
						} else {
							llvm::errs() << "Follow false branch with symbolic: \n";
		...
		case Instruction::Switch: {
			...
			if (ConstantExpr *CE = dyn_cast < ConstantExpr > (cond)) {
				...
				HSETSummary tempHSET;
				if (guilde.Path[depth] == index || guilde.Path[depth] < 0) {
					llvm::errs() << "Following switch with symbolic" << index
							<< "\n";
					tempHSET = runWithSymbolicExecution(state, guilde,
							depth + 1, abstractMethod, false);

					resultHSET.LWCET += tempHSET.LWCET;
				} else {
					llvm::errs() << "Follow switch with abstract: \n";
					HSETSummary correspondingPath = guilde.clone(0);
					correspondingPath.Path[depth] = index;
					tmpExactCachedState = HSETInfo.exactPathDictionary.find(
							Executor::ExecutionPath(correspondingPath, depth));
					if (tmpExactCachedState
							!= HSETInfo.exactPathDictionary.end()) {
						llvm::errs() << "Find explored path, reuse :"
								<< (tmpExactCachedState->second).WCET << "\n";
						tempHSET = (tmpExactCachedState->second).clone(0);
					} else
						tempHSET = runWithAbstract(state, abstractMethod, true);
				}

				resultHSET.updateNextNode(index);
				resultHSET.concat(tempHSET);
				...
				for (std::map<BasicBlock*, ref<Expr> >::iterator it =
					...
					if (es) {
						...
						if (guilde.Path[depth] == count
							...
							HSETSummary correspondingPath = guilde.clone(0);
							correspondingPath.Path[depth] = count;
							tmpExactCachedState =
									HSETInfo.exactPathDictionary.find(
											Executor::ExecutionPath(
													correspondingPath, depth));
							if (tmpExactCachedState
									!= HSETInfo.exactPathDictionary.end()) {
								llvm::errs()
										<< "Find exact explored path, reuse :"
										<< " Path " << count << " "
										<< (tmpExactCachedState->second).WCET
										<< "\n";
								tempHSET = (tmpExactCachedState->second).clone(
										0);
							} else {
								llvm::errs() << "Following path with abstract"
										<< count << "\n";
								tempHSET = runWithAbstract(*es, abstractMethod, true);
							}
	...
	if ((depth - 1) >= 0) {
		...
		//for(int i = 0; i<= depth;i++) llvm::errs()<<Executor::ExecutionPath(guilde, depth - 1).Path[i];
		std::map<ExecutionPath, HSETSummary>::iterator exactCachedState;
		exactCachedState = HSETInfo.exactPathDictionary.find(
				Executor::ExecutionPath(guilde, depth - 1));
		if (exactCachedState != HSETInfo.exactPathDictionary.end()) {
		...
		if(resultHSET.isConcrete()){
			exactCachedState = HSETInfo.exactDictionary.find(Executor::ExecutionPath(guilde, depth - 1));
			if (exactCachedState == HSETInfo.exactDictionary.end()) {
				HSETInfo.exactDictionary.insert(curExactInfo);
			}
		}
