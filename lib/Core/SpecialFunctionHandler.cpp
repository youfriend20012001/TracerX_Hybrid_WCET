...
void SpecialFunctionHandler::handleGetTaint(ExecutionState &state,
  ...
  if (executor.resolveOne(state, CE_address, op)){
    ...
    executor.bindLocal(target, state,
		...
		// Print taint details
		llvm::errs() << "\n";
		llvm::errs() << wos->getObject()->address;
		llvm::errs() << "Taint Details of " << wos->getObject()->name << "\n";

		for(unsigned k = base; k < base + size; k ++){
			ref<Expr> expr = wos->read8(k-mo->address);
			for(unsigned j = 0 ; j < expr.get()->getWidth(); j++)
			{
				llvm::errs() << "\n Bit :" << (k-base)*8 + j << "\n Tainted value: ";
				if(expr.get()->getKind() == Expr::Constant) llvm::errs() << "0";
				else
					if(expr.get()->readTaintDetails(j).isNull()) llvm::errs() << "0*";
					else
						ExprPPrinter::printSingleExpr(llvm::errs(), expr.get()->readTaintDetails(j));
				llvm::errs() << "\n";
			}
			llvm::errs() << "\n";
		}
