...
namespace {
  ...
  cl::opt<Interpreter::ReportMethod>
  PrintOut("print-out",
         cl::desc("Print execution steps"),
         cl::values(clEnumValN(Interpreter::None, "none", "Just result"),
                    clEnumValN(Interpreter::Detail, "detail", "Detail"),
  				    clEnumValN(Interpreter::Summary, "summary", "Summary"),
                    clEnumValEnd),
         cl::init(Interpreter::None));

  cl::opt<int>
    MaxInstruction("max-ins",
       cl::desc("Max instructions executed per state"),
       cl::init(10000000));

  cl::opt<int>
  MaxLoop("max-loop",
     cl::desc("Max repeat loop executed per state"),
     cl::init(10000000));

  cl::opt<int>
  MaxSplit("max-split",
       cl::desc("Max branch instruction executed per state"),
       cl::init(10000000));

  cl::opt<Interpreter::ExecutionMethod>
  ExecutionMethod("exe-method",
       cl::desc("Execution method"),
       cl::values(clEnumValN(Interpreter::SymbolicExecution, "traditional", "Traditional run"),
                  clEnumValN(Interpreter::HybridSymbolicExecution, "hybrid", "Hybrid run"),
				  clEnumValN(Interpreter::HybridAbstractExecution, "hybrid-abstract", "Hybrid abstract run"),
				  clEnumValN(Interpreter::HybridFinalExecution, "hybrid-final", "Hybrid final run"),
                  clEnumValEnd),
       cl::init(Interpreter::SymbolicExecution));
...
int main(int argc, char **argv, char **envp) {
  ...
  IOpts.ExeConfig = ExecutionMethod;
  IOpts.MaxSplit = MaxSplit;
  IOpts.MaxLoop = MaxLoop;
  IOpts.MaxInstruction = MaxInstruction;
  IOpts.PrintOut = PrintOut;
